...
const ExperienceCard: React.FC<ExperienceCardProps> = ({ company, role, date, points, logo, color }) => {
    return (
        <div className={`rounded-xl p-6 m-4 w-[100%]
  bg-white/5 backdrop-blur-md  border-black/20 
  hover:scale-105 hover:shadow-xl transition duration-300
   flex flex-col justify-between items-center text-center`}>
            <div className={`w-full text-center text-black dark:text-white rounded-t-lg p-10 text-2xl font-bold h-36 backdrop-blur-md shadow-lg ${color}`}>
                {company}
            </div>
